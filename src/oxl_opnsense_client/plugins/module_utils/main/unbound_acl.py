...
class Acl(BaseModule):
    ...
    def check(self) -> None:
        validate_str_fields(m=self.m, data=self.p, field_regex=self.STR_VALIDATIONS, allow_empty=True)

        if self.p['state'] == 'present':
            if is_unset(self.p['networks']):
                self.m.fail('You need to provide a network(s) to create an ACL!')

            for net in self.p['networks']:
                if not is_ip_or_network(net):
                    self.m.fail(f"It seems you provided an invalid network: '{net}'")
